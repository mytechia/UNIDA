...
public class UniDANotificationSuscriptionRequestMessage extends UniDADeviceMessage {
    ...
    private SuscriptionType suscriptionType = SuscriptionType.ON_CHANGE;
    ...
    public byte[] codeDeviceMessagePayload() throws MessageFormatException
    {
        ...
        try {
            ...
            byte[] suscriptionTypeData = new byte[EndianConversor.SHORT_SIZE_BYTES];
            EndianConversor.shortToLittleEndian(this.suscriptionType.getTypeValue(), suscriptionTypeData, 0);
            dataStream.write(suscriptionTypeData);
            ...
            for (int i = 0; i < this.getParams().length; i++) {
                writeString(dataStream, getParams()[i]);
            }
    ...
    protected int decodeDeviceMessagePayload(byte[] bytes, int initIndex) throws MessageFormatException {
        ...
        // suscriptionType
        this.suscriptionType = SuscriptionType.getTypeOf(bytes[offset]);
        offset += EndianConversor.BYTE_SIZE_BYTES;
    ...
    public int hashCode() {
        int hash = 7;
        hash = 43 * hash + (int) (this.notificationId ^ (this.notificationId >>> 32));
        hash = 43 * hash + (this.getDeviceID() != null ? this.getDeviceID().hashCode() : 0);
        hash = 43 * hash + (this.stateId != null ? this.stateId.hashCode() : 0);
        hash = 43 * hash + Arrays.deepHashCode(this.params);
        return hash;
    }
    ...
    public enum SuscriptionType {
        ...
        SuscriptionType(byte typeValue) {
        ...
        public static SuscriptionType getTypeOf(byte typeValue) {
            if (typeValue == ON_CHANGE.typeValue) {
                return ON_CHANGE;
            } else {
                return null;
            }
